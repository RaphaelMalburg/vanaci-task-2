...
export function ProductOverlay() {
  ...
  const hasPromotions = useMemo(() => {
    return products.some(
      (product) => product.category === "Vitaminas" || product.name.toLowerCase().includes("vitamina") || product.price < 20 // Produtos em promoção
    );
  }, [products]);
        ...
        <div className="flex flex-col h-full">
          <SheetHeader className="relative">
            ...
            {showPromotions && hasPromotions && (
              <div className="mt-3 p-3 bg-gradient-to-r from-orange-50 to-yellow-50 dark:from-orange-900/20 dark:to-yellow-900/20 rounded-lg border border-orange-200 dark:border-orange-800">
                ...
                <p className="text-xs text-orange-600 dark:text-orange-400">Produtos em destaque com preços especiais. Aproveite enquanto durarem os estoques!</p>
              </div>
            )}
          </SheetHeader>
          ...
          <div className="flex-1 overflow-y-auto mt-4">
            {isLoading ? (
              ...
              <div className="flex h-full items-center justify-center text-gray-600 dark:text-gray-300">Nenhum produto encontrado.</div>
            ) : (
              <TooltipProvider>
                <div className="space-y-4">
                        ...
                        <div className="flex flex-col items-center gap-3 flex-shrink-0">
                          ...
                          <div className="relative">
                            ...
                            <div className="absolute -top-2 -right-2 z-10">
                              <Badge variant="secondary" className="text-xs bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm border-0 shadow-sm">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex flex-col gap-3">
                            <div className="min-w-0 space-y-2">
                              ...
                              {product.description && <p className="text-sm text-gray-600 dark:text-gray-400 line-clamp-2 leading-relaxed">{product.description}</p>}
                            </div>
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2 flex-wrap">
                                <div
                                  className={`inline-flex items-center px-3 py-1.5 rounded-lg font-bold text-lg ${
                                    isOnPromotion
                                ...
                                {typeof product.stock === "number" && product.stock <= 5 && (
                                  <Badge
                                    variant="outline"
                                    className="text-xs text-amber-600 border-amber-300 bg-amber-50 dark:bg-amber-900/20 dark:text-amber-400 dark:border-amber-700">
          ...
          <div className="pt-4">
            <Button variant="outline" className="w-full" onClick={hide}>
              Fechar
            </Button>
          </div>
        </div>
