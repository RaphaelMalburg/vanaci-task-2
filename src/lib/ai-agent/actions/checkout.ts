...
export const calculateShippingTool = tool({
  ...
  execute: async ({ zipCode }: { zipCode: string }) => {
    try {
...
export const setPaymentMethodTool = tool({
  ...
  execute: async ({ method, installments }: { method: "credit" | "debit" | "pix" | "boleto"; installments?: number }) => {
    try {
...
export const placeOrderTool = tool({
  ...
  execute: async ({ paymentMethod, shippingZipCode, discountCode }: { paymentMethod: "credit" | "debit" | "pix" | "boleto"; shippingZipCode: string; discountCode?: string }) => {
    try {
