...
export const suggestProductsWithinBudgetTool = tool({
  ...
  execute: async ({ budgetAmount, category, symptomOrNeed, limit }: { budgetAmount: number; category?: string; symptomOrNeed?: string; limit: number }) => {
    ...
    if (symptomOrNeed) {
      ...
      products = products.filter((product) => {
        ...
        const symptomsMatch = product.symptoms?.some((symptom) => symptom.toLowerCase().includes(searchTerm));
        const needsMatch = product.needs?.some((need) => need.toLowerCase().includes(searchTerm));
    ...
    const productsList = suggestions.map((product) => `- ${product.name} - R$ ${product.price.toFixed(2)} (ID: ${product.id})`).join("\n");
...
export const optimizeCartForBudgetTool = tool({
  ...
  execute: async ({ budgetAmount, prioritizeEssentials }: { budgetAmount: number; prioritizeEssentials: boolean }) => {
    ...
    if (prioritizeEssentials) {
      ...
      items.sort((a, b) => {
        const aIsEssential = a.name.toLowerCase().includes("medicamento") || a.name.toLowerCase().includes("remédio") || a.name.toLowerCase().includes("vitamina");
        const bIsEssential = b.name.toLowerCase().includes("medicamento") || b.name.toLowerCase().includes("remédio") || b.name.toLowerCase().includes("vitamina");
    ...
    if (removedItems.length > 0) {
      ...
      message += removedItems.map((item) => `- ${item.name} (${item.quantity}x)`).join("\n");
      message += "\n\n";
    }

    if (suggestions.length > 0) {
      message += `Sugestões de produtos alternativos:\n`;
      message += suggestions.map((product) => `- ${product.name} - € ${product.price.toFixed(2)}`).join("\n");
    }
...
export const comparePricesTool = tool({
  ...
  execute: async ({ productName, maxResults }: { productName: string; maxResults: number }) => {
    try {
      ...
      const message =
        `Comparação de preços para "${productName}":\n\n` +
        `📊 **Estatísticas:**\n` +
...
export const compareProductPricesTool = tool({
  ...
  execute: async ({ productId, maxPrice }: { productId: string; maxPrice?: number }) => {
